...
function ContactCard({
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 1 }}
      className={`relative h-full w-full overflow-hidden backdrop-blur-xl transition-all duration-300 hover:shadow-[0_0_30px_rgba(255,45,85,0.4)] rounded-3xl ${className}`}
      style={{
      ...
      <div className="relative h-full w-full flex flex-col justify-center p-1.5 sm:p-2 md:p-3 lg:p-4 xl:p-6 gap-2 sm:gap-3 md:gap-4 lg:gap-6 xl:gap-8">
        ...
        <div className="flex items-center justify-center gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 xl:gap-6 flex-wrap">
          <a
            ...
            className="p-1.5 sm:p-2 md:p-3 lg:p-4 xl:p-5 rounded-lg sm:rounded-xl md:rounded-2xl transition-all hover:shadow-[0_0_30px_rgba(255,255,255,0.3)] hover:scale-110"
            style={{
              background: 'var(--glass-bg)',
              border: '1px solid var(--glass-border)'
            }}
          >
            <svg className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 lg:w-8 lg:h-8 xl:w-10 xl:h-10" viewBox="0 0 24 24" style={{ fill: 'var(--foreground)' }}>
...
function IOSTile({
  ...
  const content = (
    <motion.div
      ...
      whileTap={{ scale: 1 }}
      className={`relative h-full w-full overflow-hidden backdrop-blur-xl transition-all duration-300 ${colors.glow} rounded-3xl`}
      style={{
        background: 'var(--card-background)',
        border: '1px solid var(--glass-border)',
        boxShadow: 'var(--card-shadow)'
    ...
    >
        ...
        <div className="relative h-full w-full flex flex-col items-center justify-center p-1.5 sm:p-2 md:p-3 lg:p-4 text-center">
          {showImage ? (
              ...
              <div className="flex items-stretch gap-2 sm:gap-2 md:gap-3 lg:gap-4 xl:gap-6">
                ...
                <div className="flex flex-col items-start justify-between text-left flex-1">
                  ...
                  <div className="mt-0.5 sm:mt-0.5 md:mt-1 lg:mt-2 space-y-0.5 sm:space-y-0.5 md:space-y-1">
                    <p className="text-[10px] sm:text-[8px] md:text-[9px] lg:text-xs xl:text-sm leading-relaxed" style={{ color: 'var(--secondary-text)' }}>
            ...
            <>
              <Icon className="w-6 h-6 sm:w-5 sm:h-5 md:w-6 md:h-6 lg:w-8 lg:h-8 xl:w-10 xl:h-10 mb-1 sm:mb-1.5 md:mb-2" strokeWidth={1.5} style={{ color: 'var(--foreground)' }} />
              <h3 className="text-lg font-bold sm:text-[9px] sm:font-semibold md:text-[10px] lg:text-xs xl:text-sm leading-tight" style={{ color: 'var(--foreground)' }}>
                {title}
              </h3>
              {subtitle && <p className="text-xs sm:text-[8px] md:text-[9px] lg:text-[10px] mt-0.5" style={{ color: 'var(--secondary-text)' }}>{subtitle}</p>}
            </>
          )}
        </div>
      </motion.div>
...
function PageContent({ page, onBack, onContactClick }: { page: string; onBack: () => void; onContactClick?: () => void }) {
  return (
    <div className="h-screen w-screen overflow-hidden fixed inset-0" style={{ background: 'var(--gradient-bg)' }}>
      {/* Subtle grid pattern */}
      <div
